...
class TekDaysTagLib {
    ...
    void processMessages(messages, indent) {
    	messages.each{ msg -> 
    		...
    		if (children) processMessages(children, indent + 1)
    	}
    }
	...
	def loginToggle = {
		...
		if (request.getSession(false) && session.user) {
			...
			out << "<a href='${createLink(controller: 'tekUser', action: 'logout')}'>Logout</a></span>"
		} else {
			out << "<span style='float:right; margin-right:10px;'>"
			out << "<a href='${createLink(controller: 'tekUser', action: 'login')}'>Login</a>"
			out << "</span>"
		}
	...
	def organizerEvents = {
		if (request.getSession(false) && session.user) {
			...
			if (events) {
				...
				out << "<h3>Events you are organizing:</h3> <br />"
				out << "<ol>"
	...
	def volunteerEvents = {
		if (request.getSession(false) && session.user) {
			...
			if (events) {
				out << "<div style='margin-left:25px; margin-top:25px; width:85%'>"
				out << "<h3>Events you volunteered for:</h3> <br />"
				out << "<ol>"
